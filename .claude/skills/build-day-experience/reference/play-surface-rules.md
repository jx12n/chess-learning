...
The lesson lives at `/play.html` behind the profile door: seed BOTH the
registry (`chess-tutor/profiles/v1`, one selected player `p1`) and that
player's state at `chess-tutor/learner-state/v1/p1`, then
`goto('/play.html')` — see `startAtDay` in `days.spec.ts`. State is
`model.serialize(stateWhere(curriculum, model, { nodeId }))` (import
`@chess/curriculum` directly — the JSON import attribute keeps it
plain-Node loadable). Synthesis goes through the model's own API, so
seeds can never drift from the wire format. Without a registry the
lesson bounces to the landing page.

The in-app equivalent is the dev panel (`/play.html?dev=1`,
`src/devpanel.ts` — dev needs no player profile):
