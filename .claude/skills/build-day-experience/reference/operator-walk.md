...
Reach the day: open `/play.html?dev=1` and jump via the backstage
panel (isolated dev profile, real state synthesis, no player profile
needed); its sandbox and "Solve current" make single-exercise checks
fast. Walk the kid-facing items below with the panel collapsed
(Ctrl+Shift+D).
